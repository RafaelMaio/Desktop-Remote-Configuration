...
public class bezier : MonoBehaviour
{
    ...
    public AuxConfigurationMenu auxConfigureMenuScript;
    ...
    public enum Modes {
        ...
        STARTING,
        ///<summary>Training before user test</summary>
        TRAINING,
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown("space")) 
        {
            ...
            {
                if (
                    (balloon != null && boxPieceInformation.Count > 0) ||
                    (balloon == null && goS.Count == 0)
                    ) {
                    if (usabilityTestScript.getUsabilityEnabled())
                    {
                        if (balloon != null && boxPieceInformation.Count > 0)
                        {
                            usabilityTestScript.stopTime(balloon, boxPieceInformation[1]);
                        }
                        else
                        {
                    ...
                    GameObject go = Instantiate(prefabs.getResolveMap()[prefabs.getPrefabNameArray()[prefabsIdx]], Vector3.zero, Quaternion.identity);
                    go.transform.localScale = new Vector3(0.4f, 0.4f, 0.4f);

                    // Usability new Tests:
                    if (goS.Count < 2)
                    {
                        auxConfigureMenuScript.rotScaleSlider.value = 0;
                        go.GetComponent<LeanRotateCustomAxis>().enabled = false;
                    }
                    else if(goS.Count < 4)
                    {
                        go.GetComponent<LeanTranslate>().enabled = false;
                        go.GetComponent<moveY>().enabled = false;
                        auxConfigureMenuScript.rotationAxisSlider.interactable = true;
                        auxConfigureMenuScript.transformationsSlider.value = 1;
                        if(goS.Count == 2)
                        {
                            go.transform.position = new Vector3(-1.8f, 0.4f, -0.1f);
                        }
                        else
                        {
                            go.transform.position = new Vector3(4.95f, 0.14f, -1.3f);
                        }
                    }
                    else if(goS.Count < 6)
                    {
                        go.GetComponent<LeanTranslate>().enabled = false;
                        go.GetComponent<moveY>().enabled = false;
                        auxConfigureMenuScript.rotScaleSlider.value = 1;
                        auxConfigureMenuScript.transformationsSlider.value = 2;
                        if (goS.Count == 4)
                        {
                            go.transform.position = new Vector3(-1.6f, -0.43f, 2.91f);
                        }
                        else
                        {
                            go.transform.position = new Vector3(-1.60f, -0.17f, 1.81f);
                        }
                    }
                    else
                    {
                        auxConfigureMenuScript.rotScaleSlider.interactable = true;
                        auxConfigureMenuScript.rotScaleSlider.value = 0;
                        auxConfigureMenuScript.transformationsSlider.interactable = true;
                        auxConfigureMenuScript.transformationsSlider.value = 0;
                    }
                    ...
                    go.GetComponent<LeanSelectable>().Select();
                    if (auxConfigureMenuScript.mode.value == 0)
                    {
                        auxConfigureMenuScript.rotScaleSlide();
                        auxConfigureMenuScript.rotationAxisSlide();
                    }
                    else
                    {
                        auxConfigureMenuScript.transformationsSlide();
                    }
                }
            }
        }
                ...
                if (go.GetComponent<LeanSelectable>().IsSelected)
                {
                    ...
                    if (previousSelectedObject != go)
                    {
                        if (selectedArrow != null)
                        {
                            ...
                            previousSelectedObject.transform.GetChild(0).gameObject.SetActive(false);
                            previousSelectedObject.transform.GetChild(1).gameObject.SetActive(false);
                            previousSelectedObject.transform.GetChild(2).gameObject.SetActive(false);
                            previousSelectedObject.transform.GetChild(3).gameObject.SetActive(false);
                        }
                    ...
                    configureMenuScript.enableAssociation(true);
                    break;
                }
            }
        ...
        else if(mode == Modes.STARTING)
        {
            selected = true;
            selectedGameObject = aux_go;
        }
        else if(mode == Modes.BEFORE_START)
        {
            selected = true;
            selectedGameObject = trainingCube;
        }
        ...
        if (!selected)
        {
            ...
            if(balloon == null)
            {
                configureMenuScript.enableAssociation(false);
            }
        }
        else
        {
    ...
    {
        Vector3 arrowPos = go.transform.position + go.transform.GetComponent<Collider>().bounds.center + new Vector3(0, go.GetComponent<Collider>().bounds.size.y / 2 + 0.2f, go.GetComponent<Collider>().bounds.size.z / 2 + 0.2f); ;
    ...
    public void setMode(Modes _anchorsMode, bool next)
    {
        ...
        if (next)
        {
            if(mode == Modes.STARTING)
            {
                ...
                if (aux_go == null)
                {
                    auxConfigureMenuScript.rotationAxisSlider.interactable = false;
                    auxConfigureMenuScript.rotScaleSlider.interactable = false;
                    auxConfigureMenuScript.transformationsSlider.interactable = false;
            ...
            if (mode == Modes.ANCHORS)
            {
                if (usabilityTestScript.getUsabilityEnabled())
                {
                    usabilityTestScript.stopTime(aux_go, "qrcode");
            ...
            else if (mode == Modes.OBJECTS)
            {
                ...
                if (usabilityTestScript.getUsabilityEnabled())
                {
                    usabilityTestScript.stopTime(aux_go, "qrcode");
                    //aux_go.transform.position += new Vector3(0f, 1.4f, 0f);
                    //xusabilityTestScript.translateStartingPosition();
                }
                foreach (GameObject anchor in anchors)
                {
                    anchor.GetComponent<LeanSelectable>().Deselect();
                ...
                aux_go.GetComponent<LeanTranslate>().enabled = false;
                aux_go.GetComponent<LeanRotateCustomAxis>().enabled = false;
                aux_go.GetComponent<moveY>().enabled = false;
                aux_go.transform.GetChild(0).gameObject.SetActive(false);
                aux_go.transform.GetChild(1).gameObject.SetActive(false);
                aux_go.transform.GetChild(2).gameObject.SetActive(false);
                aux_go.transform.GetChild(3).gameObject.SetActive(false);
            }
        }
        else
        {
    ...
    public void changeBetweenSlideRot(int value)
    {
        if (mode == Modes.OBJECTS)
        {
            foreach (var go in goS)
            {
                if (goS.Count == 5 || goS.Count == 6 || goS.Count == 7 || goS.Count == 8)
                {
                    go.GetComponent<LeanScale>().enabled = (value == 1);
                }
                if (goS.Count == 3 || goS.Count == 4 || goS.Count == 7 || goS.Count == 8)
                {
        ...
        {
            aux_go.GetComponent<LeanScale>().enabled = (value == 1);
            aux_go.GetComponent<LeanRotateCustomAxis>().enabled = (value == 0);
            aux_go.transform.GetChild(0).gameObject.SetActive(true);
        }
        else if(mode == Modes.BEFORE_START)
        {
            trainingCube.GetComponent<LeanScale>().enabled = (value == 1);
            trainingCube.GetComponent<LeanRotateCustomAxis>().enabled = (value == 0);
            trainingCube.transform.GetChild(0).gameObject.SetActive(true);
        }
    }
    ...
    {
        if (trainingCube == null)
        {
            trainingCube = Instantiate(prefabs.getResolveMap()["Cube"], Vector3.zero, Quaternion.identity);
            trainingCube.transform.localScale = new Vector3(0.4f, 0.4f, 0.4f);
            trainingCube.transform.GetChild(0).gameObject.SetActive(true);
            trainingCube.GetComponent<LeanSelectable>().Select();
        }
        /*
        if (aux_go == null)
        {
    ...
    public void transformCube(bool cubeOrBaloon)
    {
        if (cubeOrBaloon) {
            int index = -1;
            foreach (var go in goS)
            {
                if (go.GetComponent<LeanSelectable>().IsSelected)
                {
                    balloon = Instantiate(balloonPrefab, go.transform.position, go.transform.rotation);
                    balloon.GetComponent<Orientate>().setInitialScale(go.transform.localScale);
                    balloon.transform.localScale = go.transform.localScale * 3;
                    index = goS.FindIndex(item => item == go);
        ...
        else
        {
            if (balloon != null)
            {
                GameObject newGo = Instantiate(prefabs.getResolveMap()[prefabs.getPrefabNameArray()[prefabsIdx]], balloon.transform.position, Quaternion.Euler(balloon.GetComponent<Orientate>().getInitialRotation()));
                newGo.transform.localScale = balloon.transform.localScale / 3;
                Destroy(balloon);
                goS.Add(newGo);
            }
        }
    ...
    public void componentClicked(GameObject button)
    {
        boxPieceInformation = new List<string> { button.transform.Find("ReferenceText").GetComponent<Text>().text,
                                                    button.transform.Find("NameText").GetComponent<Text>().text,
                                                    button.transform.Find("LocalizationText").GetComponent<Text>().text};
        balloon.transform.GetChild(1).GetChild(0).GetComponent<TMP_Text>().text = boxPieceInformation[0];
        balloon.transform.GetChild(1).GetChild(1).GetComponent<TMP_Text>().text = boxPieceInformation[1];
        balloon.transform.GetChild(1).GetChild(2).GetComponent<TMP_Text>().text = boxPieceInformation[2];
        ...
        {
            if (!content.transform.GetChild(i).GetComponent<Button>().interactable)
            {
                content.transform.GetChild(i).GetComponent<Button>().interactable = true;
                content.transform.GetChild(i).GetComponent<Image>().color = new Color32(47, 200, 255, 255);
                break;
            }
        }
    ...
    private void objectDone()
    {
        if (balloon != null)
        {
            GameObject newGo = Instantiate(cubeDonePrefab, balloon.transform.position, Quaternion.Euler(balloon.GetComponent<Orientate>().getInitialRotation()));
            newGo.transform.localScale = balloon.transform.localScale / 3;
            Destroy(balloon);
            goS.Add(newGo);
            boxPieceInformation.Clear();
            configureMenuScript.enabledAssociationMenu(true);
            for (int i = 0; i < content.transform.childCount; i++)
            {
                if (!content.transform.GetChild(i).GetComponent<Button>().interactable)
                {
    ...
    public void changeRotationAxis(int value)
    {
        if (mode == Modes.OBJECTS)
        {
            foreach (var go in goS)
            {
                if (goS.Count == 3 || goS.Count == 4 || goS.Count == 7 || goS.Count == 8)
                {
                    go.GetComponent<LeanRotateCustomAxis>().Axis = new Vector3(-Convert.ToSingle(value == 0), -Convert.ToSingle(value == 1), -Convert.ToSingle(value == 2));
                }
                if (go.GetComponent<LeanSelectable>().IsSelected)
                {
                    go.transform.GetChild(0).gameObject.SetActive(true);
        ...
        {
            aux_go.GetComponent<LeanRotateCustomAxis>().Axis = new Vector3(-Convert.ToSingle(value == 0), -Convert.ToSingle(value == 1), -Convert.ToSingle(value == 2));
            aux_go.transform.GetChild(0).gameObject.SetActive(true);
        }
        else if (mode == Modes.BEFORE_START)
        {
            trainingCube.GetComponent<LeanRotateCustomAxis>().Axis = new Vector3(-Convert.ToSingle(value == 0), -Convert.ToSingle(value == 1), -Convert.ToSingle(value == 2));
            trainingCube.transform.GetChild(0).gameObject.SetActive(true);
        }
    }

    public void changeTransformation(int value)
    {
        if (mode == Modes.OBJECTS)
        {
            foreach (var go in goS)
            {
                if (go.GetComponent<LeanSelectable>().IsSelected)
                {
                    go.transform.GetChild(0).gameObject.SetActive(value == -1);
                    go.transform.GetChild(1).gameObject.SetActive(value == 0);
                    go.transform.GetChild(2).gameObject.SetActive(value == 1);
                    go.transform.GetChild(3).gameObject.SetActive(value == 2);
                }
            }
        }
        else if (mode == Modes.STARTING)
        {
            aux_go.transform.GetChild(0).gameObject.SetActive(value == -1);
            aux_go.transform.GetChild(1).gameObject.SetActive(value == 0);
            aux_go.transform.GetChild(2).gameObject.SetActive(value == 1);
            aux_go.transform.GetChild(3).gameObject.SetActive(value == 2);
        }
        else if(mode == Modes.BEFORE_START)
        {
            trainingCube.transform.GetChild(0).gameObject.SetActive(value == -1);
            trainingCube.transform.GetChild(1).gameObject.SetActive(value == 0);
            trainingCube.transform.GetChild(2).gameObject.SetActive(value == 1);
            trainingCube.transform.GetChild(3).gameObject.SetActive(value == 2);
        }
    }

    public void turnTranslation(bool on)
    {
        if (mode == Modes.OBJECTS)
        {
            foreach (var go in goS)
            {
                if (goS.Count == 1 || goS.Count == 2 || goS.Count == 7 || goS.Count == 8)
                {
                    go.GetComponent<LeanTranslate>().enabled = on;
                    go.GetComponent<moveY>().enabled = on;
                }
                if (go.GetComponent<LeanSelectable>().IsSelected)
                {
                    go.transform.GetChild(0).gameObject.SetActive(on);
                }
            }
        }
        else if(mode == Modes.BEFORE_START)
        {
            trainingCube.GetComponent<LeanTranslate>().enabled = on;
            trainingCube.GetComponent<moveY>().enabled = on;
            trainingCube.transform.GetChild(0).gameObject.SetActive(on);
        }
        else if(mode == Modes.STARTING)
        {
            aux_go.GetComponent<LeanTranslate>().enabled = on;
            aux_go.GetComponent<moveY>().enabled = on;
            aux_go.transform.GetChild(0).gameObject.SetActive(on);
        }
    }
}